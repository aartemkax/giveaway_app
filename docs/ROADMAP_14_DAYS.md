...
So the current roadmap should not drift into new big features until this is handled clearly in one of two ways:

- classify and surface this state properly in product/API behavior
- or introduce a different network strategy if server-side fetch remains required

## Recommended Execution Order From Here

1. Add one more focused test around the riskiest account/job path.
